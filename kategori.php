...
$limit = 5;
if(isset($_GET['p'])){
    $noPage = $koneksi->real_escape_string($_GET['p']);
}
else $noPage = 1;

$offset = ($noPage - 1) * $limit;

$sqlKategori = "SELECT kategori FROM kategori WHERE id_kategori='".$koneksi->real_escape_string($_GET['id'])."'";

$qryKategori = $koneksi->query($sqlKategori);
...
if ($jumlah > 0) {
	...
	WHERE kategori.id_kategori = '".$koneksi->real_escape_string($_GET['id'])."'
	ORDER BY
	berita.tgl_posting DESC
	LIMIT ".$koneksi->real_escape_string($offset).",". $koneksi->real_escape_string($limit);

	$sql_rec = "SELECT id_berita FROM berita WHERE id_kategori = '".$koneksi->real_escape_string($_GET['id'])."'";
						...
						<div class="post">
							...
							<div class="row post-meta">
								<div class="col-sm-3">
									...
									<a href="<?php echo $base_url."penulis.php?id=".$post_kat['id_admin']; ?>">
										<?php echo $post_kat['nama_admin']; ?>
									</a>
								</div>
								<div class="col-sm-3">
							...
							<div class="row post-content">
								<div class="col-sm-12 excerpt">
									...
									<?php echo substr(strip_tags($post_kat['teks_berita']), 0,200) . "..."; ?>
